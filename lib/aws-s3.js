...
async function uploadGetUrl(file, name) {
    return new Promise(function(resolve, reject) {
        ...
        var urlParams = { Bucket: process.env.AWS_S3_BUCKET, Key: data.Key, Expires: process.env.AWS_S3_EXPIRY }
        s3Bucket.getSignedUrl('getObject', urlParams, function(err, url) {
            if (err) {
                ...
                console.log(url)
                resolve({ url, name: urlParams.Key });
            }
        })
    })
}
